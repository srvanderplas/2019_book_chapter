...
ggplot(data = .) + 
  ...
  scale_fill_manual("", values = c("Non-Match (KNM)" = "white", "Match (KM)" = "darkgrey")) + 
  theme_bw() + 
...
\begin{figure}
\centering
\includegraphics[width=.7\textwidth]{figure/Luger-Cutaway_greyscale.png}
...
\begin{figure}
\centering
\includegraphics[height=.25\textheight]{figure/WikiAmmo_greyscale.PNG}
...
knitr::include_graphics("figure/ComparisonMicroscope_greyscale.png")
```
...
\begin{figure}
\centering
\includegraphics[width=.3\textwidth]{figure/Bullet_greyscale.png} \includegraphics[width=.3\textwidth]{figure/CartridgeCase_greyscale.png}
...
\begin{figure}
\centering
\includegraphics[width=.8\textwidth]{figure/NIST-CMS_greyscale.png} 
...
ggplot() + 
  ...
  scale_fill_manual("", values = c("Non-Match (KNM)" = "white", "Match (KM)" = "grey10")) + 
...
knitr::include_graphics("figure/HS36-Bullet-With-Crosscut_greyscale.png")
```
...
plot_grid(
  ggdraw() + draw_image("figure/HS36-Bullet-With-Crosscut_rotate90_greyscale.png"), 
...
```{r bullet-remove-curvature, fig.cap = "Loess fit to remove curvature shown in grey (top). Residuals from the Loess fit (bottom) show the relative heights of the engraved striae.", out.width = ".8\\textwidth", fig.width = 8, fig.height = 4}
p1 <- ggplot(aes(x = x, y = value), data = filter(x3pdf, abs(y - 329.595) < 2)) +
  geom_point(size = 0.5) +
  geom_smooth(aes(x = x, y = value), data = filter(x3pdf, abs(y - 329.595) < 10, x > 110, x < 2120), color ="grey") +
  theme_bw() +
  xlim(c(0, 2300)) +
...
p1 <- b12 %>% ggplot(aes(x = land1, y = land2, fill = rfscore)) + 
  geom_tile(color = "grey90") + ggtitle ("Same barrel") +  
  scale_fill_gradient2("RF score: ", low = "white", high = "grey10", limits=c(0,1)) +
  theme_bw() + 
...
p2 <- bab %>% ggplot(aes(x = land1, y = land2, fill = rfscore)) + 
  geom_tile(color = "grey90") + ggtitle ("Different barrel") +  
  scale_fill_gradient2("RF score: ", low = "white", high = "grey10", limits=c(0,1)) +
  theme_bw() + 
  xlab ("Bullet 2") + ylab("Bullet 1") +
  theme_bw() + 
...
df2 %>% gather("case", "values", starts_with("case")) %>% 
  ...
  scale_fill_manual("Cell included in SA(A,k)", values = c("white", "grey10")) + 
  coord_equal() +
  theme_bw() +
...
f3nest %>% select(KM = KM_sam, sam_rf) %>%
  ...
  facet_grid(method~.) +
  geom_jitter() +
  scale_color_manual(guide = F, values = c("KNM" = "grey80", "KM" = "grey10")) +
...
To explore whether the random forest fitted to the \citet{hamby2009} bullets has good classification performance when used to classify pairs of land engraved areas from bullets fired from other barrels or by guns of different make and model, and when the ammunition is also made by a different manufacturer, \citet{hare2017algorithmic} applied the model to thousands of pairs of known matching and known non-matching land engraved areas obtained from crime laboratories across the United States. Results have been promising; the random forest correctly determined every pair of bullets in every study when the gun barrel had conventional (rather than polygonal) rifling and striations were reasonably well marked, and when bullets were not coated with a polymer that flakes on contact with the barrel. 
 \autoref{fig:score-match-plot} shows the results observed in one such test set. Each pairwise comparison is represented as a square, with the center of the square shaded to represent the composite match score for the six lands on each bullet; the border of the square is colored dark grey if the bullets are from the same source. In the test set shown in \autoref{fig:score-match-plot}, there are three reference shots from each of eight barrels, and 10 fired rounds of unknown provenance. The test set was open, which means that it was possible that some of the test shots were not fired by any of the eight study barrels and that one or more of the barrels may have fired none of the 10 test shots; in this case, questioned bullets Q, Y, and Z did not originate from the 8 barrels in the study.
...
p3 <- pdnest %>%
  ...
  scale_fill_gradient2("RF score ",
    low = "white",
    high = "darkgrey", midpoint = 0.45, limits = c(0, 1)
  ) +
  scale_colour_manual("Same source", values = "grey10") +
  scale_y_discrete() +
  theme_bw() +
  theme(legend.position = "bottom") +
   # coord_equal() +
...
```{r l2l-scores, fig.width = 8, fig.height = 4, out.width = ".8\\textwidth", fig.cap = "Empirical distributions of random forest scores for pairs of land engraved area scans known to have been produced from the same land (grey) and known to have originated from different lands (white). The vertical lines in the top and bottom panels represent the score that was obtained when comparing a questioned with a reference scan of a land engraved area."}
...
p <- tmp %>% 
  ...
  scale_color_manual("", values = c("lightgrey", "grey10")) +
