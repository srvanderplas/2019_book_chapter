...
p1 <- b12 %>% ggplot(aes(x = land1, y = land2, fill = rfscore)) + 
  ...
  scale_fill_gradient2("RF score: ", low = "white", high = "darkgrey", limits=c(0,1)) +
  theme_bw() + 
...
f3nest %>% select(KM = KM_sam, sam_rf) %>%
  ...
  facet_grid(method~.) +
  geom_jitter() +
  scale_color_manual(guide = F, values = c("KNM" = "grey80", "KM" = "grey20")) +
...
p3 <- pdnest %>%
  ...
  scale_colour_manual("Same source", values = "grey20") +
  scale_y_discrete() +
  theme_bw() +
  theme(legend.position = "bottom") +
   # coord_equal() +
...
ggplot() +
  ...
  scale_color_manual("", values = c("darkgrey", "grey80")) + 
...
p + 
  geom_line(data = tmpnest$model[[2]], 
            aes(x = x, y = y), size=1.25, colour="grey20", fill=NA) +
