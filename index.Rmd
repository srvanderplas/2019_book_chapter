...
=======
```{r marks, fig.cap = "Left panel: Bullet showing engraved land areas and groove impressions. Right panel:  Bottom of a cartridge case.  EM denotes ejector marks, FP denotes firing pin impression and BF denotes breech face impressions.  \\footnotesize{Cartridge case image downloaded from NIST, via Wikimedia Commons, with slight modifications (\\url{https://commons.wikimedia.org/wiki/File:Cartridge_Case_(7788259910).jpg})}.", out.width = ".75\\textwidth"}
lst <- c("figure/Bullet.png", "figure/CartridgeCase-recolor.png") %>%
>>>>>>> 5b7cebbf00b04178d80343ce01e1e7500dc46ae9
  lapply(., load.image)

lst[[2]] <- imresize(lst[[2]], scale = pmin(height(lst[[1]])/height(lst[[2]]), width(lst[[1]])/width(lst[[2]]))) %>%
  pad(10, axes = "x", pos = -1, val = "white") %>%
  pad(pmax(0, height(lst[[1]]) - height(.)), axes = "y", pos = 0, val = "white") %>%
  pad(pmax(0, width(lst[[1]]) - width(.)), axes = "x", pos = 0, val = "white")
